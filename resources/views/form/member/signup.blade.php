<form action="{{ route('member.store') }}" method="POST">
   ...
   <div class="grid grid-cols-1 2xl:grid-cols-2 gap-3">
      <div class="flex flex-col mb-2">
         <label for="postcode" class="sr-only">Opcinski broj (BiH)</label>
         <input type="text" name="postcode" id="postcode" class="form-control basic-input @error('postcode') border border-red-500 @enderror" placeholder="Opcinski broj (BiH) *" value="{{ old('postcode') }}">
         @error('postcode')<div class="error-label">{{ $message }}</div>@enderror
      </div>
      <div class="flex flex-col mb-5">
         <label for="city" class="sr-only">Opcina (BiH)</label>
         <input type="text" name="city" id="city" class="form-control basic-input @error('city') border border-red-500 @enderror" placeholder="Opcina (BiH) *" value="{{ old('city') }}">
   ...
   <div class="flex items-center">
      ...
      <div class="ml-1">
         <label for="accept-statut" class="dark:text-white @error('accept-statut') text-red-500 @enderror">Prihvatam <a href="{{ asset('static/assets/files/Statut-UPD-Povratak.pdf') }}" class="link" target="_blank">
               Statut, viziju i ciljeve</a> Udruzenja
         </label>
      </div>
